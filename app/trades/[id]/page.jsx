  ...
  return (
    <div>
      <div className="flex between" style={{ marginBottom: 12 }}>
        <h1>{trade.instrument} {trade.side.toUpperCase()} — {trade.level || '(no level)'}</h1>
        <div className="flex">
      ...
      <div className="grid-2">
        ...
        <div className="card">
          ...
          <Row label="Exit price">
            <input value={trade.exit} onChange={e => update({ exit: e.target.value })} />
          </Row>
          <Row label="R multiple">
            <input value={trade.rMultiple} onChange={e => update({ rMultiple: e.target.value })} placeholder="e.g. 2.5" />
          </Row>
          <Row label="PnL ($)">
            <input value={trade.pnl} onChange={e => update({ pnl: e.target.value })} />
          </Row>
          <Row label="Outcome">
          ...
          <Row label="Severin discipline">
            <label className="flex" style={{ gap: 8 }}>
              <input
                type="checkbox"
                checked={!!trade.deriskedAt2_5R}
                onChange={e => update({ deriskedAt2_5R: e.target.checked })}
                style={{ width: 'auto' }}
              />
              De-risked at 2.5R
            </label>
          ...
          <Row label="">
            <label className="flex" style={{ gap: 8 }}>
              <input
                type="checkbox"
                checked={!!trade.movedStopToBE}
                onChange={e => update({ movedStopToBE: e.target.checked })}
                style={{ width: 'auto' }}
              />
              Stop moved to BE
            </label>
          </Row>
          <Row label="Runner closed at">
            <input value={trade.runnerClosedAt} onChange={e => update({ runnerClosedAt: e.target.value })} />
          </Row>

          <Row label="Lesson">
            <textarea value={trade.lesson} onChange={e => update({ lesson: e.target.value })} placeholder="What did the market teach you? Process vs. outcome." />
          </Row>
...
function KV({ k, v }) {
  return (
    <div className="flex" style={{ justifyContent: 'space-between', padding: '4px 0', borderBottom: '1px dashed var(--border)' }}>
